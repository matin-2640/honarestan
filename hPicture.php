...
<body class="news-page">
                    ...
                    <?php if (count($images) > 0): ?>
                      <?php foreach ($images as $index => $img): ?>
                        <img class="index-gallery-img slide-img <?php echo $index === 0 ? 'active' : ''; ?>"
                          src="<?php echo htmlspecialchars($img['image_path'], ENT_QUOTES); ?>" alt="<?php echo $album_title; ?>"
  ...
  <div id="imageModal"
    ...
    <button type="button" onclick="modalChangeSlide(-1)"
      style="position:absolute; right:20px; background:rgba(255,255,255,0.2); border:none; color:white; font-size:24px; padding:10px 15px; cursor:pointer; border-radius:50%;">&#10094;</button>

    <img id="modalImage" src="" alt="تصویر بزرگ"
      style="max-width:80%; max-height:85vh; object-fit:contain; border-radius:8px;">

    <button type="button" onclick="modalChangeSlide(1)"
      style="position:absolute; left:20px; background:rgba(255,255,255,0.2); border:none; color:white; font-size:24px; padding:10px 15px; cursor:pointer; border-radius:50%;">&#10095;</button>
  </div>
